...
class SAC(OffPolicyAgent):
    ...
    def get_action(self, 
        ...
        if init_state is None:
            action, *res = self._get_action_body(tf.constant(state), 
                                                 mask, 
                                                 init_state, 
                                                 map_state, 
                                                 test,
            ...
            action, _, _, h = self._get_action_body(tf.constant(state), 
                                                    mask, 
                                                    init_state, 
                                                    map_state, 
                                                    test,
    ...
    def train(self, 
        ...
        if self.actor_critic:
            td_errors, actor_loss, vf_loss, qf_loss, q_value, logp_min, logp_max, logp_mean, entropy_mean,traj,pred_loss = train_func(
                ...
                map_state,next_map_state,hist_traj, future_state,future_map_state ,future_action, vision=vision, next_vision=next_vision)
        else:
            td_errors, actor_loss, vf_loss, qf_loss, q_value, logp_min, logp_max, logp_mean, entropy_mean,traj,pred_loss = train_func(
                states, actions, next_states, rewards, dones, weights,mask,hidden,next_mask,next_hidden,ego,ego_mask,
                map_state,next_map_state,hist_traj, vision=vision, next_vision=next_vision)

        return td_errors,traj,pred_loss

    def train_rep(self,state,map_state,future_state,future_map_state,future_action, vision=None, next_vision=None):
        return self._represent_train(state,map_state,future_state,future_map_state,future_action, vision=vision, next_vision=next_vision).numpy()

    @tf.function
    def _represent_train(self,
                         state,
                         map_state,
                         future_state,
                         future_map_state,
    ...
    @tf.function
    def _train_body_actor_critic(self, 
                                 states, 
                                 actions, 
                                 next_states, 
                                 rewards, 
        ...
        with tf.device(self.device):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                current_q,current_q_2 = self.qf(states, actions, mask=mask, init_state=hidden, 
                                                map_state=map_state, vision=vision)

                current_h = self.qf._get_hidden(states, 
                                                mask=mask, 
                                                init_state=hidden, 
                                                map_state=map_state, 
                                                vision=vision)
                ...
                next_h = self.qf._get_hidden(next_states, mask=next_mask, init_state=next_hidden, 
                                             map_state=next_map_state, vision=next_vision)
                
                next_actions, next_logp, _ = self.actor(next_h)
  
                target_q, target_q_2 = self.qf_target(next_states, 
                                                     next_actions,
                                                     mask=next_mask,
                ...
                if self.aug:
                    current_q_a,current_q_2_a = self.qf(states, 
                                                        actions,
                                                        mask=mask,
                                                        init_state=hidden,
                                                        map_state=map_state,
                    ...
                    next_h_a = self.qf._get_hidden(next_states,
                                                   mask=next_mask,
                                                   init_state=next_hidden,
                                                   map_state=next_map_state,
                                                   aug=False,
                    ...
                    target_q_a,target_q_2_a = self.qf_target(next_states, 
                                                             next_actions_a,
                                                             mask=next_mask,
                                                             init_state=next_hidden,
                                                             map_state=next_map_state,
                ...
                if self.representations and not self.sep_rep_opt:
                    simi_loss =  self.rep_func(states,
                                               map_state,
                                               future_action,
                                               future_state,
                                               future_map_state,
                ...
                sample_q,sample_q_2 = self.qf(states, 
                                              sample_actions,
                                              mask=mask,
                                              init_state=hidden,
                                              map_state=map_state,
